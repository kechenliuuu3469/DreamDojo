#!/bin/bash
#SBATCH --job-name=lam_language_table
#SBATCH --nodes=1
...
mkdir -p slurm_outputs/lam_language_table
mkdir -p exp_ckpts_language_table
mkdir -p exp_imgs_language_table
...
echo "Config: config/lam_language_table.yaml"
...
torchrun \
    ...
    main.py fit \
    --config config/lam_language_table.yaml \
    --data.num_workers=4 \
