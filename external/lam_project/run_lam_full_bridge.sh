#!/bin/bash
#SBATCH --job-name=lam_bridge_full
#SBATCH --nodes=1
...
PYTHONWARNINGS=ignore PYTHONUNBUFFERED=1 torchrun \
    --nnodes=1 \
    --nproc_per_node=4 \
    main.py fit \
    --config config/lam_bridge_full.yaml \
    --data.num_workers=4 \
    --ckpt_path /scratch/gpfs/AM43/users/kl0820/projects/DreamDojo/external/lam_project/exp_ckpts_bridge_full/last.ckpt
