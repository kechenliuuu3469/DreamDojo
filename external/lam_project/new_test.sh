#!/bin/bash
#SBATCH --job-name=lam_droid_full_test
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --ntasks=1
...
mkdir -p slurm_outputs/lam_droid_full_test
mkdir -p exp_ckpts_droid_full_test
mkdir -p exp_imgs_droid_full_test
...
echo "Config: config/lam_droid_fulltest.yaml"
echo "=========================================="
...
torchrun \
    ...
    --nproc_per_node=8 \
    main.py fit \
    --config config/lam_droid_fulltest.yaml \
    --data.num_workers=4 \
    --pretrained_ckpt $BIG/DreamDojo/checkpoints/LAM/LAM_400k.ckpt
